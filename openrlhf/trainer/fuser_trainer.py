...
class FuserTrainer(ABC):
    ...
    def __init__(
        ...
        self.flag_weight = getattr(self.args, "flag_weight", 0.5)
        self.comp_weight = getattr(self.args, "comp_weight", 0.5)
    ...
    def fit(self, args, consumed_samples=0, num_update_steps_per_epoch=None):
        ...
        for epoch in range(start_epoch, self.epochs):
            ...
            # train
            self.model.train()
            for inputs, attention_masks, flag_loss_masks, comp_loss_masks in self.train_dataloader:
                inputs = inputs.to(torch.cuda.current_device()).squeeze(1)
                attention_mask = attention_masks.to(torch.cuda.current_device()).squeeze(1)
                flag_loss_mask = flag_loss_masks.to(torch.cuda.current_device()).squeeze(1)
                comp_loss_mask = comp_loss_masks.to(torch.cuda.current_device()).squeeze(1)

                per_token_log_probs, _ = self.model(
                    inputs,
                ...
                flag_loss_sum += flag_loss.item()
                comp_loss_sum += comp_loss.item()
                ...
                if step % self.strategy.accumulated_gradient == 0:
                    logs_dict["weighted_loss_mean"] = weighted_loss_sum / self.strategy.accumulated_gradient
                    weighted_loss_sum = 0
                    logs_dict["flag_loss_mean"] = flag_loss_sum / self.strategy.accumulated_gradient
                    flag_loss_sum = 0
                    logs_dict["comp_loss_mean"] = comp_loss_sum / self.strategy.accumulated_gradient
                    comp_loss_sum = 0
    ...
    def evaluate(self, eval_dataloader, steps=0):
        ...
        with torch.no_grad():
            ...
            for inputs, attention_masks, flag_loss_masks, comp_loss_masks in eval_dataloader:
                inputs = inputs.to(torch.cuda.current_device()).squeeze(1)
                attention_mask = attention_masks.to(torch.cuda.current_device()).squeeze(1)
                flag_loss_mask = flag_loss_masks.to(torch.cuda.current_device()).squeeze(1)
                comp_loss_mask = comp_loss_masks.to(torch.cuda.current_device()).squeeze(1)
                ...
                comp_loss_sum += comp_loss.item()
                bar_dict = {
                    "eval-weighted_loss": weighted_loss_sum / times,
                    "eval-flag_loss": flag_loss_sum / times,
                    "eval-comp_loss": comp_loss_sum / times
                }
                step_bar.update()
